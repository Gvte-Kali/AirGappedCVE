...
# =============================================================================
# ⚙️ CONFIGURATION (TOUT EST CHARGÉ DEPUIS .env)
# =============================================================================
# --- Sauvegarde locale ---
...
title() {
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') - [TITLE] $1" >> "$LOG_FILE"
}
...
# =============================================================================
# 🗑️ ROTATION DES BACKUPS (TOUS NOMS CONFONDUS)
# =============================================================================
rotate_backups() {
    local backup_dir=$1
    local max_backups=$2
    ...
    if [ \"$BACKUP_COUNT\" -ge \"$max_backups\" ]; then
        ...
        if [ -n \"$OLDEST_BACKUP\" ]; then
            ...
            warn \"Aucun backup ancien trouvé à supprimer dans $backup_dir.\"\n        fi
    fi
}
...
backup_to_nas_ssh() {
    ...
    info \"Copie du backup vers $NAS_SSH_USER@$NAS_SSH_SERVER:$nas_backup_path...\"\n    if scp -P \"$NAS_SSH_PORT\" \"$LOCAL_BACKUP_FILE\" \"$NAS_SSH_USER@$NAS_SSH_SERVER:$nas_backup_path\"; then
        ...
        info \"Rotation des backups sur le NAS...\"\n        ssh -p \"$NAS_SSH_PORT\" \"$NAS_SSH_USER@$NAS_SSH_SERVER\" \
            \"cd \\\"$(dirname \"$nas_backup_path\")\\\" && \
            find . -type f -name '*.sql.gz' -printf '%T@ %p\\n' | sort -n | head -n -$MAX_BACKUPS | cut -d' ' -f2- | xargs -I {} rm -f {} 2>/dev/null\" || \
...
backup_to_nas_rsync() {
    ...
    info \"Copie du backup vers $NAS_RSYNC_USER@$NAS_RSYNC_SERVER::$nas_backup_path...\"\n    if rsync -avz -e ssh \"$LOCAL_BACKUP_FILE\" \"$NAS_RSYNC_USER@$NAS_RSYNC_SERVER::$nas_backup_path\"; then
        ...
        info \"Rotation des backups sur le NAS...\"\n        ssh \"$NAS_RSYNC_USER@$NAS_RSYNC_SERVER\" \
            \"cd \\\"$(dirname \"$nas_backup_path\")\\\" && \
            find . -type f -name '*.sql.gz' -printf '%T@ %p\\n' | sort -n | head -n -$MAX_BACKUPS | cut -d' ' -f2- | xargs -I {} rm -f {} 2>/dev/null\" || \
            warn \"Rotation des backups sur le NAS impossible. Faites-le manuellement.\"
    else
...
show_summary() {
    ...
    if [ \"$ENABLE_NAS_BACKUP\" = \"true\" ]; then
        ...
        case \"$NAS_PROTOCOL\" in
            smb)   echo -e \"${CYAN}Serveur:${NC} $NAS_SMB_SERVER/$NAS_SMB_SHARE/$NAS_SMB_DIR\" ;;
            ssh)   echo -e \"${CYAN}Serveur:${NC} $NAS_SSH_SERVER:$NAS_SSH_PORT$NAS_SSH_DIR\" ;;
            rsync) echo -e \"${CYAN}Serveur:${NC} $NAS_RSYNC_SERVER::$NAS_RSYNC_DIR\" ;;
            local) echo -e \"${CYAN}Chemin:${NC} $NAS_LOCAL_MOUNT\" ;;
        esac
    fi
