...
# Demander si on veut supprimer la base de données
...
if [[ "$DB_CONFIRM" =~ ^[oOyY]$ ]]; then
    if command -v mariadb >/dev/null 2>&1; then
        info "Suppression de la base de données $DB_NAME..."
        mariadb -u root -e "DROP DATABASE IF EXISTS $DB_NAME; DROP USER IF EXISTS '$DB_USER'@'localhost'; DROP USER IF EXISTS '$DB_USER'@'%'; FLUSH PRIVILEGES;" 2>/dev/null
        if [ $? -eq 0 ]; then
...
if [ -f "$USER_BASHRC" ]; then
    if grep -q "asset-manager" "$USER_BASHRC"; then
        ...
        sed -i '/# Ajouté par install.sh - AirGappedCVE/d,/export PATH.*asset-manager/d' "$USER_BASHRC"
